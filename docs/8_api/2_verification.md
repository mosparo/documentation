...
| `metadata`              | Object  | An object that contains all metadata that was saved in mosparo for the submission. It is added to the response if the option in the advanced settings is enabled. _(Added in v1.5)_                           |
...
**Method**: POST<br />
**Endpoint**: /api/v1/verification/store-metadata<br />
**Content-Type**: application/json (payload as JSON string in the request body)

This API endpoint can be used to store additional metadata in the submission after processing the form data. You can use this, for example, to store the information on how the submission was processed.
...
| `submitToken`         | String | Required | The submit token was requested by the JavaScript script in the frontend. |
| `validationSignature` | String | Required | The HMAC SHA256 hash of the validation token.                            |
| `metadata`            | Object | Required | An object with all metadata that should be stored.                       |
...
| Name     | Type    | Description                                                                                                            |
|----------|---------|------------------------------------------------------------------------------------------------------------------------|
| `result` | Boolean | If everything worked as expected, this property is set and is `true`. If there was a problem, the property is not set. |
