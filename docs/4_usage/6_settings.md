...
| Secret key                   | The secret key is there to sign the interface requests and ensure that the requests are genuine. |

## Project members

Project members are users who  have access to a project. The users can have different permissions and sometimes only view projects, but do not change anything. Userscanbe added as project members in more than one project.
...
The minimum time can be used to specify how much time must elapse between the initial request and the verification of the form before the request is accepted. Bots can submit form requests within seconds, during which time a normal user takes several minutes.

Please note that some users are faster and others slower in typing. The minimum time should be as short as possible in order not to erroneously reject correct requests.
...
As soon as field a is populated, the submission is considered spam because the normal user does not see the field. For users with screen readers, the field is marked with a note that it should not be filled in.
...
:::caution
Users can share the sameIP address, for example in a multi-user apartment or in a company.
:::
...
| Detection period           | Defines the time in seconds in which the requests must be made before the request delay becomes active.                                 |
| Request delay time         | Defines the time, in seconds, that the user must wait before sending a new request.                                                     |
| Multiplier                 | The multiplier is applied to the request delay time and increases (or decreases) the delay time for re-requests within that delay time. |

#### Example

:::note Example values
Number of allowed requests: 30, Detection period: 30 sec, Request delay time: 60 sec, Multiplier: 1.5
:::

- A user makes 30 requests within 20 seconds. The automatic delay becomes active and the user has to wait for 60 seconds.
- If the user makes another request within these 60 seconds, the time increases from 60 to 90 seconds with the help of the factor.
- For a further request within the 90 seconds, the time increases to 135 seconds.

### Automatic Lock

If an IP address sends a large number of requests, an IP address can be automatically blocked for a certain period of time with automatic blocking. The lock is made automatically and takes effect immediately. As soon as the set time has elapsed, the IP address can receive a submission code again.
...
:::caution
IP addresses are blocked throughout the entire mosparo installation. If an IP address has been blocked in one project, it is automatically blocked in another project if both projects have autoblock enabled.
:::
...
| Field                      | Description                                                                                                                           |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| Number of allowed requests | The field defines the number of requests a user can make within a period of time before the IP block becomes active.                  |
| Detection period           | Defines the time, in seconds, in which the requests must be made before the IP block becomes active.                                  |
| Initial lock time          | Defines the time, in seconds, that the user is locked and cannot send requests.                                                       |
| Multiplier                 | The multiplier is applied to the initial lock time and increases (or decreases) the lock time for new requests within that lock time. |

#### Example

:::note Example values
Number of allowed requests: 30, Detection period: 30 sec, Initial blocking time: 300 sec, Multiplier: 1.5
:::

- A user makes 30 requests within 20 seconds. The IP block becomes active and the user is banned for 300 seconds.
- If the user makes another request within these 300 seconds, the time increases from 300 to 450 seconds using the factor.
- For a further request within these 450 seconds, the time increases to 675 seconds.

### List of allowed IP addresses

If you want certain IP addresses to be excluded from the request delay and automatic blocking, you can add these IP addresses or subnets to the list of allowed IP addresses. The security features are skipped for theIP addresses and subnets enlisted on it. However, the normal check for spam still takes place.

:::caution
Users can share the same IP address, for example in an apartment with multiple users or in a company.
:::
...
When the user focuses on the control box with the keyboard or the user moves the mouse over the mosparo box, the mosparo logo is displayed. If you do not want this branding, you can disable the display of the logo.

### Ping animation

In order to draw attention to the control box, a ping animation is built into the standard system, which should highlight the checkbox. If you do not want this animation, you can disable it at any time.

### Delivery of Display Settings
...
Our goal is that everyone can fill out your form. For this it is important for us that the mosparo box can also be used by a person who does not see as well as other people. It is therefore important that the colors you choose have the highest possible contrast. For this reason, we have integrated a function directly in the display settings, which calculates the corresponding contrasts and displays them to you.
