...
| `forceInvisible`           | Boolean\|Null | `null`                                | _(Mainly for multi-step forms)_ Unless you are on the last step, the mosparo box should be initialized as an invisible box so that we can display the overlay for saving the data and proceed to the next step. If this parameter is not set (or is `null`), mosparo automatically uses `isMultiStepForm` and `isLastStep` to determine whether the mosparo box should be displayed. Default logic: `(isMultiStepForm && !isLastStep)` |
| `inputFieldSelector`       | String        | `[name]:not(.mosparo__ignored-field)` | Defines the selector with which the fields are searched.                                                                                                                                                                                                                                                                                                                                                                              |
| `isMultiStepForm`          | Boolean       | `false`                               | Set this to true if the initialization is for a multi-step form.                                                                                                                                                                                                                                                                                                                                                                      |
| `isLastStep`               | Boolean       | `false`                               | _(For multi-step forms)_ With this parameter, you tell the mosparo box that when submitting this step, mosparo should verify the data and not just store the data. This is a simple logical comparison like: `activeStep == lastStep`.                                                                                                                                                                                                |
...
| `submitToken`              | String        | _empty_                               | _(For multi-step forms)_ In the first step, this parameter is empty (or not set). But in the other steps, you need to set this to the submit token you've received from the previous step.                                                                                                                                                                                                                                            |
